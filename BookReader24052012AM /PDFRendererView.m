...
- (void) setPdfDocument:(CGPDFDocumentRef) value {
    ...
    //For BookReader
    self.pagePdfNumber = 1;
}


@synthesize pageNumber;
@synthesize pagePdfNumber;
...
// For BookReader
- (void) setPagePdfNumber:(int)value {
    pagePdfNumber = value;
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.bounds.size.width > self.bounds.size.height) {
        ...
        if (pagePdfNumber == 1) {
            [self drawPDFPage:CGPDFDocumentGetPage(pdfDocument, 0) inRect:rect usingContext:context];
        }
        else {
            [self drawPDFPage:CGPDFDocumentGetPage(pdfDocument, pagePdfNumber) inRect:rect usingContext:context];
        }
        ...
        if (pagePdfNumber == 1) {
            [self drawPDFPage:CGPDFDocumentGetPage(pdfDocument, 1) inRect:rect usingContext:context];
        }
        else {
            [self drawPDFPage:CGPDFDocumentGetPage(pdfDocument, pagePdfNumber + 1) inRect:rect usingContext:context];
        }
         */
	} else {
